...
class RadarSimWrapper:
    ...
    def _setup_function_signatures(self):
        ...
        self.lib.Add_Mesh_Target.argtypes = [
            ctypes.POINTER(ctypes.c_float),  # points
            ctypes.POINTER(ctypes.c_int),  # cells
            ctypes.c_int,  # cell_size
            ctypes.POINTER(ctypes.c_float),  # origin
            ctypes.POINTER(ctypes.c_float),  # location
            ctypes.POINTER(ctypes.c_float),  # speed
            ctypes.POINTER(ctypes.c_float),  # rotation
            ctypes.POINTER(ctypes.c_float),  # rotation_rate
            ctypes.c_float,  # ep_real
            ctypes.c_float,  # ep_imag
            ctypes.c_float,  # mu_real
            ctypes.c_float,  # mu_imag
            ctypes.c_bool,  # skip_diffusion
            ctypes.c_float,  # density
            ctypes.c_bool,  # environment
            ctypes.c_void_p,  # ptr_targets_c
        ]
        self.lib.Add_Mesh_Target.restype = ctypes.c_int

        # Get baseband buffer size
        self.lib.Get_BB_Size.argtypes = [ctypes.c_void_p]
        self.lib.Get_BB_Size.restype = ctypes.c_int
        ...
        self.lib.Run_RadarSimulator.restype = ctypes.c_int
    ...
    def create_fmcw_waveform(
        self, center_freq: float, bandwidth: float, duration: float, samples: int
    ...
    def create_antenna_pattern(
        self, num_points: int = 361, max_gain_db: float = 15.0
    ) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def run_example(self):
        ...
        freq, freq_time = self.create_fmcw_waveform(
            center_freq, bandwidth, duration, samples
        )
        ...
        pulse_start_time_c = pulse_start_time.ctypes.data_as(
            ctypes.POINTER(ctypes.c_double)
        )

        # Create transmitter
        print("2. Creating transmitter...")
        tx = self.lib.Create_Transmitter(
            freq_c,
            freq_time_c,
            samples,
            freq_offset_c,
        ...
        buffer_size = self.lib.Get_BB_Size(radar)
        ...
        ret = self.lib.Run_RadarSimulator(
            radar,
            targets,
            0,
            1.0,
        ...
        if ret != 0:
            raise RadarSimError(f"Radar simulation failed with error code {ret}")
